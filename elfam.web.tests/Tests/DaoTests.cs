...
namespace elfam.web.Tests
{
    public class DaoTests
    {
        ...
        {
            return PostgreSQLConfiguration.Standard.ConnectionString("Server=localhost;Database=elfam;User Id=postgres;Password=111111;");
...
//            c3.Subcategories.Add(c34);
        ...
        public void AddUids()
        {
            ...
            {
                list.Add(NumberUtils.Convert((ulong)i));
            }
                    ...
                    Product product = new Product()
                                          {
                                              ...
                                              ShortDescription = shortDescriptions[random.Next(shortDescriptions.Length)]
                                          };
...
//            return;
        ...
        {
            ISession session = GetConfiguration().BuildSessionFactory().OpenSession();
            ProductAdminListSearchCriteria criteria = new ProductAdminListSearchCriteria();
            //criteria.CategoryId = 1;
            criteria.SortExpression = "p.Name";
            ProductAdminListQueryBuilder builder = new ProductAdminListQueryBuilder(criteria);
            var list = builder.Execute(session);
            foreach (var VARIABLE in list)
            {
...
//            names.Add(OrderStatus.WaitPickup);
        ...
        public void TestLinq()
        {
            ...
            var query = 
                ...
                select new {Product = p, Summ = p.Incomes.Sum(x => x.QuantityCurrent)};
